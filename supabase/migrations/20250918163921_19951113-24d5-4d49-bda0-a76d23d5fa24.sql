-- Générer des paiements fictifs pour les souscriptions historiques entièrement payées
-- Cela corrigera le problème d'historique de paiement vide et d'affichage incohérent

-- Fonction pour générer les paiements manquants pour les souscriptions historiques
...
BEGIN
  ...
  FOR sub_record IN 
    ...
    WHERE s.type_souscription IN ('mise_en_garde', 'historique')
      AND s.solde_restant = 0 -- Seulement celles marquées comme entièrement payées
  LOOP
...
-- Exécuter la génération des paiements manquants
SELECT * FROM public.generate_missing_historical_payments();
-- Update calculate_location_dette function to handle historical vs new contracts
...
BEGIN
  ...
  -- Calculer la dette selon le type de contrat et l'année
  IF NEW.type_contrat = 'historique' THEN
    -- Anciens locataires: 12 mois complets chaque année (pas d'avance payée)
    dette_annuelle = (annees_ecoules * NEW.loyer_mensuel * 12) + 
                     (mois_dans_annee_courante * NEW.loyer_mensuel);
  ELSE
    -- Nouveaux locataires: logique actuelle avec avance de 2 mois
